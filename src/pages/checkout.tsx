...
const CheckoutPage = () => {
  ...
  const handleCheckout = async () => {
    ...
    axios.get(`https://upgrade.fineasapp.io:2053/get-user-info?id_hash=${profile?.id_hash}`).then((res) => {
      console.log(res.data['user'])
      
    })

    try {
      const response = await axios.post('https://upgrade.fineasapp.io:2053/upgrade_membership', {
  ...
  const handleCancellation = async () => {
    try {
      const response = await axios.post('https://upgrade.fineasapp.io:2053/cancel-subscription', {
