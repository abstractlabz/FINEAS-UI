...
const Chat: React.FC = () => {
  ...
  const fetchChatNames = async (profile: UserProfile | null) => {
    try {
      const response = await axios.get('http://62.3.50.146:5600/get-chat-names', { params: { id_hash: profile?.id_hash } });
...
const checkCreditsAndSendMessage = async () => {
  ...
  try {
    const creditCheckResponse = await axios.post('http://62.3.50.146:5600/enforce-credits', { id_hash: profile?.id_hash });
  ...
  const saveChat = async () => {
    ...
    try {
      ...
      await axios.post('http://62.3.50.146:5600/savechat', {
        chatname: chatName,
  ...
  const loadChat = async (name: string) => {
    ...
    try {
      ...
      const response = await axios.post('http://62.3.50.146:5600/loadchat', {
        chatname: name,
        id_hash: profile?.id_hash
      });
